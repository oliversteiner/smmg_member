...
class UnsubscribeForm extends FormBase
{
    ...
    {
        return 'smmg_member_unsubscribe_form';
    }
        ...
        if (!$this->member_nid) {
            ...
            if ($email == '') {
                ...
                if (FALSE === $valiated_email) {
                    ...
                    // Valid Email Address
                    $member_nid = MemberController::isEmailInUse($email);
                    if (!$member_nid) {
    ...
    {
        $result = MemberController::unsubscribe($this->member_nid);
        dpm($result);
        ...
        if ($result['token']) {
            ...
            $arg['nid'] = $result['nid'];
            $form_state->setRedirect('smmg_member.bye', $arg);
